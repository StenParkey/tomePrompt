...
// Sub Components
import coinFlipApp from './sub_components/coinFlip.js';
import eightBallApp from './sub_components/eightBall.js';
import diceRollApp from './sub_components/diceRoll.js'
...
export default async function diviUtils(user) {
	...
	switch (utilSelectionObject.selection) {
		case 'coinflip':
			const coinFlipAppSession = await coinFlipApp(user);
			break;
		case '8ball':
			const eightBallAppSession = await eightBallApp(user);
			break;
		case 'diceroll':
			const diceRollAppSession = await diceRollApp(user);
			break;
		case 'tarotdeck':
